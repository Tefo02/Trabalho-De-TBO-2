...
public:
  ...
  void compress(const std::string &inputFilePath,
                const std::string &outputFilePath) {
    CharacterReader reader;
    reader.fileReading(inputFilePath);
    auto frequencies = reader.getFrequencias();
    if (frequencies.empty()) {
      std::cout << "Arquivo de entrada vazio ou nao encontrado. Nada a fazer."
                << std::endl;
      return;
    }
    ...
    outputFile.write(reinterpret_cast<const char *>(&paddingInfo),
                     sizeof(paddingInfo));

    uint16_t uniqueChars = frequencies.size();
    outputFile.write(reinterpret_cast<const char *>(&uniqueChars),
                     sizeof(uniqueChars));

    for (const auto &pair : frequencies) {
      outputFile.write(&pair.first, sizeof(pair.first));
      outputFile.write(reinterpret_cast<const char *>(&pair.second),
                       sizeof(pair.second));
    }
    ...
    while (inputFile.get(character)) {
      ...
      if (huffmanCodes.find(character, code)) {
        for (char bit : code) {
          ...
          if (bitCount == 8) {
            outputFile.write(reinterpret_cast<const char *>(&byteBuffer),
                             sizeof(byteBuffer));
            byteBuffer = 0;
            bitCount = 0;
          }
    ...
    std::cout << "Arquivo comprimido com sucesso em: " << outputFilePath
              << std::endl;
  }

  void decompress(const std::string &inputFilePath,
                  const std::string &outputFilePath) {
    std::ifstream inputFile(inputFilePath, std::ios::binary);
    std::ofstream outputFile(outputFilePath, std::ios::binary);
    ...
    if (uniqueChars == 0) {
      inputFile.close();
      outputFile.close();
      std::cout << "Arquivo de entrada nao contem dados para descompressao."
                << std::endl;
      return;
    }
    ...
    if (root == nullptr) {
      std::cout
          << "Nao foi possivel reconstruir a arvore a partir do cabecalho."
          << std::endl;
      return;
    }
    ...
    for (size_t i = 0; i < fileData.size(); ++i) {
      ...
      int bitsToProcess =
          (i == fileData.size() - 1 && paddingInfo != 8) ? paddingInfo : 8;
    ...
    std::cout << "Arquivo descomprimido com sucesso em: " << outputFilePath
              << std::endl;
  }
};
