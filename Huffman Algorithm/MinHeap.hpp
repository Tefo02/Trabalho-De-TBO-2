...
private:
  ...
  void swap(int index1, int index2) {
    if (index1 >= 0 && index1 < heap.size() && index2 >= 0 &&
        index2 < heap.size()) {
      T aux = heap[index1];
  ...
  // Move o elemento no índice 'i' para baixo até restaurar a propriedade do
  // heap
  void heapifyDow(int i) {
    ...
    if (i != index) {
      swap(i, index);
      heapifyDow(index); // Se o menor não for o nó atual,chamada recursiva para
